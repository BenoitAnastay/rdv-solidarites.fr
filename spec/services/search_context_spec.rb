...
describe SearchContext, type: :service do
  ...
  context "mocking Users::GeoSearch" do
  ...
  describe "#current_step" do
    context "with an agent_id and current_user" do
      it "returns service selection" do
        agent = create(:agent)
        user = create(:user, agents: [agent])
        search = described_class.new(user, { agent_id: agent.id })
        expect(search.current_step).to eq(:motif_selection)
      end
    end

    context "with an agent_id without current_user" do
      it "returns motif selection" do
        agent = create(:agent)
        search = described_class.new(nil, { agent_id: agent.id })
        expect(search.current_step).to eq(:service_selection)
      end
    end
  end
end
